...
Aim of this project is to assess the potential of Rstudios' 'QUARTO'-package for reproducible reporting of registry data.

### Methods

Dummy data was obtained using the Framingam data-frame ('riskCommunicator'-package). Dummy department_IDs were created. Please refer to the QUARTO-file for the code.
...
The table above can easily be adapted to create an overview for a Clinic report by adding "by clinic" to the code. Furthermore, I added the option "p-value":
...
> > ***Note2**: p-values were calculated using the default-options. For an actual report I may consider using t-tests e.g. if N>100 vs. Wilcoxon-test for 10<N<100. Of course this needs to be programmed flexibly as I do not no the N a priori. 


## Graph 2.1. Density plot of age 
A density plot showing the age-distribution in my clinic vs. all other clinics is shown below: 
